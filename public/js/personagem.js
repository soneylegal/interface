...
class Personagem {
  ...
  set vida(novaVida) {
    if (novaVida >= 0) {
      ...
      console.log("Encerrar jogo");
    }
  }

  set habilidade1(h) { this.#habilidade1 = h }
  set habilidade2(h) { this.#habilidade2 = h }
...
function criarPersonagens() {
  const lampiao = new Personagem("Lampião", "Cangaceiro", 100, 20, 15, 50, pistola, tiroduplo)
  const francisctexeira = new Personagem("Francisc Teixeira", "Jagunço", 90, 10, 20, 40, faca, chute)
  const volante = new Personagem("Volante", "Policial", 110, 15, 12, 60, faca, pistola)
  const bandidoscidade = new Personagem("Bandido da Cidade", "Bandido", 80, 5, 18, 30, disparo, faca)
  const bandidoscidade2 = new Personagem("Bandido da Cidade", "Bandido", 80, 5, 18, 30, disparo, faca)
  const zerufino = new Personagem("Zé Rufino", "Coronel", 120, 25, 10, 100, escopeta, facao)

  return { lampiao, francisctexeira, volante, bandidoscidade, bandidoscidade2, zerufino }
}
